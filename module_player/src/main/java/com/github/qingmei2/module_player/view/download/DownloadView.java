...
import com.github.qingmei2.module_player.view.sectionlist.SectionedRecyclerViewAdapter;
...
public class DownloadView extends FrameLayout implements OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void deleteDownloadInfo() {
        ...
        if (alivcDownloadingMediaInfos.size() <= 0) {
            sectionAdapter.removeSection(DownloadSection.DOWNLOADING_TAG);
        }

        if (alivcDownloadMediaInfos.size() <= 0) {
            sectionAdapter.removeSection(DownloadSection.DOWNLOADED_TAG);
        }
    ...
    public void addAllDownloadMediaInfo(List<AliyunDownloadMediaInfo> alldownloadMediaInfos) {
        ...
        for (AliyunDownloadMediaInfo downloadMediaInfo : alldownloadMediaInfos) {
            String tag = downloadMediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.Complete ? DownloadSection.DOWNLOADED_TAG : DownloadSection.DOWNLOADING_TAG;
    ...
    public void addDownloadMediaInfo(AliyunDownloadMediaInfo downloadMedia) {
        ...
        String tag = downloadMedia.getStatus() == AliyunDownloadMediaInfo.Status.Complete ? DownloadSection.DOWNLOADED_TAG : DownloadSection.DOWNLOADING_TAG;
        ...
        if (sectionAdapter.getSection(tag) == null) {
            ...
            section.setOnSectionItemClickListener(new DownloadSection.OnSectionItemClickListener() {
                ...
                public void onItemClick(int posion, String tag) {
                    ...
                    if (tag.equals(DownloadSection.DOWNLOADING_TAG)) {
                    ...
                    } else if (tag.equals(DownloadSection.DOWNLOADED_TAG)) {
                    ...
                    if (tag.equals(DownloadSection.DOWNLOADED_TAG)) {
                        // 点击播放下载完成的视频
